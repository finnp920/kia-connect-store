...
function updateKVSection(theme) {
    ...
    const titleText = isMobile && theme.mobileKvTitle ? theme.mobileKvTitle : theme.kvTitle;
    const subtitleText = isMobile && theme.mobileKvSubtitle ? theme.mobileKvSubtitle : theme.kvSubtitle;
    ...
    const kvImages = isMobile && theme.mobileKvImages ? theme.mobileKvImages : theme.kvImages;
    ...
    if (slidesContainer) {
        ...
        const clones = Array.from(allSlides).filter(s => s.classList.contains('clone'));
...
function updateThemeSelectors() {
    document.querySelectorAll('.theme-selector').forEach(selector => {
        selector.classList.toggle('selected', selector.dataset.theme === currentTheme);
    });
}
...
function updateSectionThemesBackground() {
    ...
    if (sectionThemes) {
        // 모든 테마 클래스 제거
        sectionThemes.classList.remove('theme-baby-animals', 'theme-space-wonders', 'theme-landscape');
        // 현재 테마 클래스 추가
    ...
    if (sectionPlayful) {
        // 모든 테마 클래스 제거
        sectionPlayful.classList.remove('theme-baby-animals', 'theme-space-wonders', 'theme-landscape');
        // 현재 테마 클래스 추가
    ...
    if (themeHeaderWrapper) {
        // 모든 테마 클래스 제거
        themeHeaderWrapper.classList.remove('theme-baby-animals', 'theme-space-wonders', 'theme-landscape');
        // 현재 테마 클래스 추가
...
function updateDiscoverShots(theme) {
    ...
    if (isMobile && theme.mobileDiscoverShots && theme.mobileDiscoverShots.length > 0) {
    ...
    videoItems.forEach((item, index) => {
        ...
        if (thumbnail) {
            thumbnail.src = thumbnailSources[thumbnailIndex] || thumbnailSources[0] || '';
        }
...
function updateDiscoverFeatures(theme) {
    ...
    if (playfulTitle) {
        ...
        playfulTitle.innerHTML = features.title.replace(' ', brHTML ? ` ${brHTML}` : ' ');
    }
    ...
    if (videoDescription) videoDescription.textContent = features.videoDescription;
...
function updateReasonImages(theme) {
    ...
    const reasonImages = [
        { id: 'reasonCardImage1', pc: theme.kvImages?.[0] || 'assets/images/kv-slide-1.png', mobile: 'assets/images/mobile-reason-1.png' },
        { id: 'reasonCardImage2', pc: 'assets/images/reasons-space-wonders.png', mobile: 'assets/images/mobile-reason-2.png' },
        { id: 'reasonCardImage3', pc: 'assets/images/reasons-landscape.png', mobile: 'assets/images/mobile-reason-3.png' },
        { id: 'reasonCardImage4', pc: 'assets/images/reasons-car-display.png', mobile: 'assets/images/mobile-reason-4.png' }
    ];
...
function setupEventListeners() {
    document.querySelectorAll('.option-item').forEach(item => {
        item.addEventListener('click', () => handleOptionSelect(item.dataset.theme));
    });

    document.querySelectorAll('.theme-card[data-theme]').forEach(card => {
        card.addEventListener('click', () => handleOptionSelect(card.dataset.theme));
    });

    document.querySelectorAll('.theme-selector[data-theme]').forEach(selector => {
        selector.addEventListener('click', () => handleOptionSelect(selector.dataset.theme));
    });
    ...
    if (prevArrow) prevArrow.addEventListener('click', () => navigateCarousel('prev'));

    const nextArrow = document.querySelector('.kv-arrow-next');
    if (nextArrow) nextArrow.addEventListener('click', () => navigateCarousel('next'));
...
function updateCarousel(noTransition = false) {
    ...
    const realIndex = currentSlide === 0 ? realSlideCount - 1 : 
                      currentSlide === realSlideCount + 1 ? 0 : 
                      currentSlide - 1;
