...
// Global State
// ----------------------------------------
let natgeo_themeNames = Object.keys(natgeo_themeData);
let currentTheme = natgeo_themeNames[0];
let natgeo_swiper = null;
let natgeo_isLiked = false;
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if (urlTheme && natgeo_themeData[urlTheme]) {
...
function initializePage() {
  const theme = natgeo_themeData[currentTheme];
  if (!theme) {
...
function handleOptionSelect(themeId) {
  ...
  natgeo_themeNames = theme.themeNames;
}
...
function handleLikeClick() {
  ...
  natgeo_isLiked = !natgeo_isLiked;
  btnLike.classList.toggle('active', natgeo_isLiked);

  console.log('Like status:', natgeo_isLiked);
}
